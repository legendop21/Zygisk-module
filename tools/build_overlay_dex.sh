...
rm -rf "$OUT_DIR/classes" "$OUT_DIR/overlay.jar" "$OUT_DIR/overlay.dex"
...
(
  cd "$OUT_DIR/classes"
  jar cf "$OUT_DIR/overlay.jar" com/
)

"$D8" --lib "$ANDROID_JAR" --min-api 26 --output "$OUT_DIR" "$OUT_DIR/overlay.jar"
mv -f "$OUT_DIR/classes.dex" "$OUT_DIR/overlay.dex"
rm -rf "$OUT_DIR/classes" "$OUT_DIR/overlay.jar" "$OUT_DIR/sources.txt"
